...
# This is the same file as in mmdet3d/tools/data_converters/nuscenes_converter.py
...
def _fill_trainval_infos(nusc,
    ...
    for sample in mmengine.track_iter_progress(nusc.sample):
        ...
        if not test:
            ...
            # add poses for each agent: pose in world coordinate system
            # we can also estimate the pose from gt_boxes, 
            # but it will assume pitch and roll are 0
            gt_poses = []
            for anno in annotations:
                pose = np.eye(4)
                rotation = Quaternion(anno['rotation']).rotation_matrix
                translation = np.array(anno['translation'])
                pose[:3, :3] = rotation
