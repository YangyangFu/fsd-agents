...
MODELS.register_module(module=MultiheadAttention, name='MultiheadAttention')
...
class SpatialCrossAttention(BaseModule):
    ...
    def __init__(self,
        ...
        self.deformable_attention = MODELS.build(deformable_attention) # build_attention()
...
class MultiScaleDeformableAttention3D(BaseModule):
    ...
    def __init__(self,
        ...
        self.batch_first = batch_first
        self.output_proj = None # will be added in spatial_cross_attention
    ...
    def forward(self,
                ...
                **kwargs) -> torch.Tensor:
        """Forward Function of MultiScaleDeformAttention3D.
        Args:
            query (Tensor): Query of Transformer with shape
                ( bs, num_query, embed_dims).
            key (Tensor): The key tensor with shape
    ...
    def forward(self,
        ...
        Args:
            ...
            key_pos (Tensor): The positional encoding for `key`. Default
                None.
            reference_points (Tensor):  The normalized reference
                points with shape (2*bs, num_query, num_levels, 2),
                all elements is range in [0, 1], top-left (0,0),
                bottom-right (1, 1), including padding area. The 2*bs is due 
                to the fact that (bev_query, bev_feats) are concatenated. E.g., 
                (2, 40000, 1, 2)
