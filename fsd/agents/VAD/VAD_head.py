...
from .utils import (normalize_2d_pts, 
                   ...
                   denormalize_2d_pts, 
                   denormalize_2d_bbox)
from .VAD_transformer import inverse_sigmoid
...
class VADHead(DETRHead):
    ...
    def __init__(self,
        ...
        if kwargs['train_cfg'] is not None:
            ...
            self.map_assigner = TASK_UTILS.build(map_assigner)
            # DETR sampling=False, so use PseudoSampler
            sampler_cfg = dict(type='PseudoSampler', _scope_='mmdet')
            self.map_sampler = TASK_UTILS.build(sampler_cfg)
        
        # architecture parameters
        #self.embed_dims = transformer.embed_dims
        self.num_decoder_layers = 1
        self.num_map_decoder_layers = 1
        self.num_motion_decoder_layers = 1
        ...
        #    self.num_map_decoder_layers = transformer.map_decoder.num_layers
        if self.bev_decoder is not None:
            self.num_decoder_layers = self.bev_decoder.num_layers
        if self.map_decoder is not None:
            self.num_map_decoder_layers = self.map_decoder.num_layers
        ...
        self.map_decoder = MODELS.build(map_decoder)
        self.motion_decoder = MODELS.build(motion_decoder)
        self.motion_map_decoder = MODELS.build(motion_map_decoder)
        self.ego_agent_decoder = MODELS.build(ego_agent_decoder)
        self.ego_map_decoder = MODELS.build(ego_map_decoder)
        
        # build positional encoding
        self.positional_encoding = MODELS.build(positional_encoding) # bev positional encoding
    ...
    def _init_layers(self):
        #TODO: What is this classification for?
        ...
        # turning left, straight, turning right or k modes
        traj_cls_branch = []
        for _ in range(self.num_reg_fcs):
        ...
        motion_num_pred = (self.num_motion_decoder_layers + 1) if \
            self.as_two_stage else self.num_motion_decoder_layers
        map_num_pred = (self.num_map_decoder_layers + 1) if \
            self.as_two_stage else self.num_map_decoder_layers
        ...
        # agent/agent, agent/map interaction
    ...
    def init_weights(self):
        ...
        if self.bev_encoder is not None:
            for p in self.bev_encoder.parameters():
                if p.dim() > 1:
                    nn.init.xavier_uniform_(p)
            self._init_attention_modules(self.bev_encoder.modules())
        
        if self.bev_decoder is not None:
            for p in self.bev_decoder.parameters():
                if p.dim() > 1:
                    nn.init.xavier_uniform_(p)
            self._init_attention_modules(self.bev_decoder.modules())
        
        if self.map_decoder is not None:
            for p in self.map_decoder.parameters():
                if p.dim() > 1:
                    nn.init.xavier_uniform_(p)
            self._init_attention_modules(self.map_decoder.modules())
    ...
    def get_bev_map_latent(self,
        ...
        if self.bev_decoder is not None:
            # [L, Q, B, D], [L, B, Q, D]
            inter_states, inter_references = self.bev_decoder(
                query=query,
                key=None,
                value=bev_embed,
                query_pos=query_pos,
    ...
    def forward(self,
        ...
        # if only return bev features, return it directly
        ...
        outputs = self.get_bev_map_latent(
            mlvl_feats,
            bev_embed,
            bev_queries,
            object_query_embeds,
        ...
        if self.motion_decoder is not None:
            ...
            if self.motion_map_decoder is not None:
                ...
                # NOTE: cross-attention motion query
        ...
        # ego features for decoding trajectories
        # [ego_agent_query, ego_map_query, ego_local_context_feat]
