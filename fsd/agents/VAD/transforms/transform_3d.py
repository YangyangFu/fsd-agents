    ...
    def __call__(self, results):
        ...
        for key in [
            ...
            if isinstance(results[key], list):
                results[key] = BaseDataElement([to_tensor(res) for res in results[key]])
            else:
...
class DefaultFormatBundle3D(DefaultFormatBundle):
    ...
    def __call__(self, results):
        ...
        if 'points' in results:
            assert isinstance(results['points'], PointData)
    ...
    def __call__(self, results):
        ...
        if self.with_ego:
            ...
            if 'ego_fut_masks' in results:
                results['ego_fut_masks'] = to_tensor(results['ego_fut_masks'][None, None, ...])
            if 'ego_fut_cmd' in results:
                results['ego_fut_cmd'] = to_tensor(results['ego_fut_cmd'][None, None, ...])
            if 'ego_lcf_feat' in results:
                results['ego_lcf_feat'] = to_tensor(results['ego_lcf_feat'][None, None, ...])
