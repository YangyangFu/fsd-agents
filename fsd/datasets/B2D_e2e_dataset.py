...
from mmengine.fileio.io import load, dump
...
class B2D_E2E_Dataset(Planning3DDataset):
    ...
    def union2one(self, queue):
        imgs_list = [each['img'].data for each in queue] # list of tensor with shape [multi_view, C, H, W]
        gt_labels_3d_list = [each['gt_labels_3d'].data for each in queue] # class index of each object in the lidar point cloud
        gt_sdc_label_list = [each['gt_sdc_label'].data for each in queue]
        gt_inds_list = [to_tensor(each['gt_inds']) for each in queue] #TODO: what is this?? instance IDs in lidar or camera???
        gt_bboxes_3d_list = [each['gt_bboxes_3d'].data for each in queue] # list of LidarInstance3DBoxes for each object (N, 9)
        gt_past_traj_list = [to_tensor(each['gt_past_traj']) for each in queue] # TODO: (4, 8, 2) for idx=0, (n_pasts, ?, 2)
        gt_past_traj_mask_list = [ to_tensor(each['gt_past_traj_mask']) for each in queue] # (4, 8, 2)
        gt_sdc_bbox_list = [each['gt_sdc_bbox'].data for each in queue] # list of LidarInstance3DBoxes for ego vehicle, (1, 9)
        ...
        if 'gt_future_labels' in queue[-1]: # None
        ...
        for i, each in enumerate(queue):
            ...
            if metas_map[i]['folder'] != prev_scene_token: # no previous scene for the first frame
                ...
                # normalize the position and angle with respect to the previous frame
        ...
        queue = queue[-1]
        queue['gt_labels_3d'] = BaseDataElement(data=gt_labels_3d_list)
        queue['gt_sdc_label'] = BaseDataElement(data=gt_sdc_label_list)
        queue['gt_inds'] = BaseDataElement(data=gt_inds_list)
        queue['gt_bboxes_3d'] = BaseDataElement(data=gt_bboxes_3d_list)
        queue['gt_sdc_bbox'] = BaseDataElement(data=gt_sdc_bbox_list)
        queue['l2g_r_mat'] = BaseDataElement(data=l2g_r_mat_list)
        queue['l2g_t'] = BaseDataElement(data=l2g_t_list)
        queue['timestamp'] = BaseDataElement(data=timestamp_list)
        queue['gt_fut_traj'] = BaseDataElement(data=gt_fut_traj)
        queue['gt_fut_traj_mask'] = BaseDataElement(data=gt_fut_traj_mask)
        queue['gt_past_traj'] = BaseDataElement(data=gt_past_traj_list)
        queue['gt_past_traj_mask'] = BaseDataElement(data=gt_past_traj_mask_list)
        if gt_future_boxes_list is not None:
            queue['gt_future_boxes'] = BaseDataElement(data=gt_future_boxes_list)
        if gt_future_labels_list is not None:
            queue['gt_future_labels'] = BaseDataElement(data=gt_future_labels_list)
    ...
    def get_data_info(self, index):
        ...
        """
        # anno info: 
        # ['folder', 'town_name', 'command_far_xy', 'command_far', 'command_near_xy', 
        # 'command_near', 'frame_idx', 'ego_yaw', 'ego_translation', 'ego_vel', 'ego_accel', 
        # 'ego_rotation_rate', 'ego_size', 'world2ego', 'brake', 'throttle', 'steer', 
        # 'sensors', 'gt_ids', 'gt_boxes', 'gt_names', 'num_points', 'npc2world']
        info = self.data_infos[index] 

        # gt_names: class names of the objects -> class name in the dataset
        ...
        # map info: lane masks, lane labels, lane bounding boxes
    ...
    def get_ann_info(self, index):
        ...
        mask = (past_mask.sum((1,2))>0).astype(np.int_)
